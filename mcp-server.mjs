...
const SESSION_FILE = join(tmpdir(), 'dsh-cu-mcp-session.json');
const SHOT_FILE = join(tmpdir(), 'dsh-cu-last-shot.png');
...
const INPUT_SCHEMAS = {
  ...
  scroll: {
    ...
    properties: {
      ...
      element_index: { type: 'integer' }, pages: { type: 'number' }, x: { type: 'number' }, y: { type: 'number' },
    },
  },
