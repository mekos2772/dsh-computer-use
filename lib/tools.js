...
const APP_PARAM = {
  type: 'string',
  description:
    'App name, process name (chrome, notepad.exe), PID number, window title substring, or native HWND number.',
};
  ...
  function snapMarkerPoint(cx, cy) {
    ...
    for (const el of elements) {
      ...
      const interactive = el.settable === true
        || (Array.isArray(el.secondaryActions) && el.secondaryActions.includes('Invoke'));
      if (!interactive) continue;
      const f = el.frame;
      if (!f || cx < f.x || cx > f.x + f.width || cy < f.y || cy > f.y + f.height) continue;
      if (Math.min(f.width, f.height) > cap) continue;   // too big to be a click target
  ...
  return [
    ...
    // ------------------------------------------------------------ get_app_state
    {
      ...
      description:
        ...
        'This must be called once per assistant turn before interacting with the app. ' +
      ...
      async execute(args, exec) {
        const result = await invokePowerShell({
          ...
          lastPoint: session.lastScreenPoint ?? null,   // screen px; kernel maps window-relative
        });
    ...
    // ------------------------------------------------------------------- click
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Clicked (${value.via ?? 'coordinate'})`, value.x != null ? `at (${value.x}, ${value.y})` : '']),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('click', [target], (motion) => invokePowerShell({
            action: 'click',
            app: args.app,
            signal: exec?.signal,
            path,
    ...
    // -------------------------------------------------- perform_secondary_action
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Performed secondary action via ${value.via ?? '?'}`, value.error ? `(${value.error})` : '']),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('perform_secondary_action', [elementPoint(args.element_index)], () => invokePowerShell({
    ...
    // --------------------------------------------------------------- set_value
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Set value via ${value.via ?? '?'}`]),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('set_value', [elementPoint(args.element_index)], () => invokePowerShell({
          action: 'set_value', app: args.app, path, value: args.value, signal: exec?.signal,
        }), { pulse: false });
      },
    },
    ...
    // ------------------------------------------------------------ select_text
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Selected text via ${value.via ?? '?'}`, value.note ? `(${value.note})` : '']),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('select_text', [elementPoint(args.element_index)], () => invokePowerShell({
            action: 'select_text',
            app: args.app,
            signal: exec?.signal,
            path,
    ...
    // ----------------------------------------------------------------- scroll
    {
      ...
      parameters: {
        ...
        pages: { type: 'number', description: 'Number of pages to scroll. Fractional values supported. Defaults to 1.' },
      ...
      output: {
        ...
        render: (_args, value) => summary([`Scrolled ${value.direction ?? ''}`, value.pages != null ? `${value.pages} pages` : '', `(${value.via ?? 'mouse wheel'})`]),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('scroll', [target], (motion) => invokePowerShell({
            action: 'scroll',
            app: args.app,
            signal: exec?.signal,
            path,
    ...
    // ------------------------------------------------------------------ drag
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Dragged via ${value.via ?? 'mouse_event'}`]),
      },
      async execute(args, exec) {
        ...
        return withPointerMotion('drag', [dragStart, dragEnd], (motion) => invokePowerShell({
            action: 'drag',
            app: args.app,
            signal: exec?.signal,
    ...
    // -------------------------------------------------------------- press_key
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Pressed ${value.key ?? ''}`]),
      },
      async execute(args, exec) {
        await requireApproval(ctx, exec, 'press_key', `Press ${args.key} in ${args.app}`);
        return invokePowerShell({ action: 'press_key', app: args.app, key: args.key, signal: exec?.signal });
      },
    },
    ...
    // -------------------------------------------------------------- type_text
    {
      ...
      output: {
        ...
        render: (_args, value) => summary([`Typed ${JSON.stringify(_args.text)} via ${value.via ?? 'clipboard'}`]),
      },
      async execute(args, exec) {
        await requireApproval(ctx, exec, 'type_text', `Type ${JSON.stringify(args.text)} into ${args.app}`);
        return invokePowerShell({ action: 'type_text', app: args.app, text: args.text, signal: exec?.signal });
      },
    },
